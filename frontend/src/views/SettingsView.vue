...
import { KeyRound, LockKeyhole, Pencil, Plus, RefreshCw, ShieldCheck, UserPlus, Users } from 'lucide-vue-next'
...
<template>
  <div class="content compact settings-page">
    ...
    <section v-else class="settings-grid"><article class="settings-panel"><div class="settings-panel-head"><div><strong>发布执行策略</strong><span>全局默认值，项目和目标服务器可以覆盖具体命令</span></div><ShieldCheck /></div><div class="settings-info-list"><div><strong>真实执行模式</strong><span>发布会通过 SSH 登录目标服务器，执行构建、上传、启动和健康检查。</span></div><div><strong>失败自动回滚</strong><span>文件部署回切 current；Docker 和 Compose 恢复上一版本容器或目录。</span></div><div><strong>生产发布确认</strong><span>生产环境的发布操作应由发布人员二次确认。</span></div></div></article><article class="settings-panel"><div class="settings-panel-head"><div><strong>当前运行参数</strong><span>修改这些参数需要通过部署环境变量完成</span></div></div><div class="settings-env-list"><code>EXECUTOR_MODE=real</code><code>SSH_CONNECT_TIMEOUT_SECONDS=10</code><code>BUILD_TIMEOUT_SECONDS=1800</code><code>SSH_COMMAND_TIMEOUT_SECONDS=600</code></div><small class="settings-hint">敏感密钥和执行器参数暂不开放在网页中修改，避免误操作导致平台无法启动。</small></article></section>
